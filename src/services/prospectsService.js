...
export function docToDisplay(doc) {
  ...
  const prospectName =
    getAttr(doc, "fullName", "fullname", "FullName") ||
    getAttrByKeyMatch(doc, "fullName", "fullname") ||
    getAttr(doc, "name", "Name") ||
    getAttrByKeyMatch(doc, "name", "Name") ||
    "-";
  return {
    id: doc.$id ?? doc.id ?? "",
    name: prospectName,
    address:
      ...
      getAttrByKeyMatch(doc, "address", "Address") ||
      "-",
    phoneNumber:
      getAttr(doc, "mobile", "Mobile", "phoneNumber", "Phone") ||
      getAttrByKeyMatch(doc, "mobile", "Mobile", "phoneNumber", "Phone") ||
      "-",
    badgeId:
      getAttr(doc, "badgeId", "BadgeId") ||
      getAttrByKeyMatch(doc, "badgeId", "BadgeId") ||
      getAttr(doc, "batchNumber", "BatchNumber") ||
      getAttrByKeyMatch(doc, "batchNumber", "BatchNumber") ||
      "-",
    assignedTo:
      getAttr(doc, "assignedTo", "AssignedTo") ||
      getAttrByKeyMatch(doc, "assignedTo", "AssignedTo") ||
      "Unassigned",
    bloodGroup:
...
function toDbProspect(p) {
  ...
  return {
    ...
    mobile:
      String(p.mobile ?? p.phoneNumber ?? p.mobileNumber ?? "").trim() || "",
    ...
    DeptFinalisedName:
      String(p.DeptFinalisedName ?? p.departmentName ?? "").trim() || "",
    ...
    namdaanInitiated:
      String(
        p.namdaanInitiated ?? (p.initiated ? "yes" : "no") ?? "no",
      ).trim() || "no",
    NamdaanInitiationBy:
      String(p.NamdaanInitiationBy ?? p.initiationBy ?? "").trim() || "",
    NamdaanInitiationPlace:
      String(p.NamdaanInitiationPlace ?? p.initiationPlace ?? "").trim() || "",
  };
}
