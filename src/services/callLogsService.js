...
export async function listCallLogsForUser(submittedBy) {
  ...
  try {
    ...
    try {
      ...
      const filtered = (response.documents || []).filter((d) => String(d.submittedBy || '').trim() === String(submittedBy).trim())
...
export async function listCallLogsForProspect(prospectId) {
  ...
  try {
    ...
    console.error('Failed to list call logs for prospect (indexed query):', error)
    ...
    try {
      ...
      const filtered = (response.documents || []).filter((d) => String(d.prospectId || '').trim() === String(prospectId).trim())
      return { documents: filtered, total: filtered.length }
    } catch (fallbackError) {
      console.error('Fallback list call logs for prospect failed:', fallbackError)
      return { documents: [], total: 0 }
    }
  }
}
...
export async function listCallLogsWithNominalList() {
  ...
  try {
    const response = await databases.listDocuments(databaseId, callLogsCollectionId, [
      Query.equal('nominalListSelect', 'Yes'),
      Query.orderDesc('$createdAt'),
    ])
    return response
  } catch (error) {
    ...
    try {
      ...
      const filtered = (response.documents || []).filter((d) => String(d.nominalListSelect || '').trim().toLowerCase() === 'yes')
      return { documents: filtered, total: filtered.length }
    } catch (fallbackError) {
...
export async function listCallLogsWithVisitYes() {
  ...
  try {
    ...
    try {
      ...
      const filtered = (response.documents || []).filter((d) => String(d.visitSelect || '').trim().toLowerCase() === 'yes')
      return { documents: filtered, total: filtered.length }
    } catch (fallbackError) {
...
export async function listCallLogsForJathaRecord() {
  ...
  try {
    ...
    const filtered = docs.filter((d) => {
      const nominalYes = String(d.nominalListSelect || '').trim().toLowerCase() === 'yes'
      const visitYes = String(d.visitSelect || '').trim().toLowerCase() === 'yes'
...
export async function updateCallLog(logId, data) {
  ...
  const updated = await databases.updateDocument(databaseId, callLogsCollectionId, logId, updates)
  return updated
}
